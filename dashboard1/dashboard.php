...
// Define o limite de estoque considerado "baixo"
$limite_estoque_baixo = 10; // Ajuste este valor conforme necessário

// Consulta produtos com estoque baixo
$query_estoque_baixo = "SELECT id, nome, estoque FROM produtos WHERE estoque <= ?";
$stmt_estoque = $conn->prepare($query_estoque_baixo);
$stmt_estoque->bind_param('i', $limite_estoque_baixo);
$stmt_estoque->execute();
$resultado_estoque_baixo = $stmt_estoque->get_result();

// Armazena produtos com estoque baixo
$produtos_estoque_baixo = [];
$total_notificacoes = 0;

if ($resultado_estoque_baixo && $resultado_estoque_baixo->num_rows > 0) {
    while ($produto = $resultado_estoque_baixo->fetch_assoc()) {
        $produtos_estoque_baixo[] = $produto;
    }
    $total_notificacoes = count($produtos_estoque_baixo);
}
$stmt_estoque->close();
